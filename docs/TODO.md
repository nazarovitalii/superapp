...
| F-12a | 🔴        | Обязательный полный адрес (leaf): пока leaf не выбран — вниз разворачиваются селекты локаций следующего уровня (поиск если >10 вариантов, селект если ≤10)                                                                         | `[ ]`  |
| F-12b | 🔴        | Бегунок видимости адреса: по умолчанию 100%, точки = уровни локаций (напр. Damac Hills → Golf Town → Golf Promenade → GP 5 → GP 5a). Можно двигать влево, но НЕ ниже уровня комьюнити. В ленте всем показывается выбранный уровень | `[ ]`  |
| F-12c | 🟡        | Occupied → поле «занято до» (месяц+год, необязательное); `lease_until` если occupancy_status=Occupied                                                                                                                              | `[ ]`  |
| F-12d | 🟡        | Официальный листинг → обязательное прикрепление документации (Form A PDF, как в старой форме React-проекта)                                                                                                                        | `[ ]`  |
| F-12e | 🟡        | После выбора leaf — Building info из `location_developers` (год постройки, этажность и т.д.) автоматически блоком ниже                                                                                                             | `[ ]`  |
| F-12f | 🔴        | Шаг выбора комнат: 2 чекбокса — `is_maid` «Maid room» и `is_hotel_pool` «Hotel apartment». ⚠️ Решение 2026-06-12: подтип «Hotel Apartment» как тип объекта УБИРАЕМ — вместо него только чекбокс is_hotel_pool (поле в БД уже есть) | `[ ]`  |

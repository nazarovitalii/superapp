...
> ⚠️ **Важно (уточнено 2026-06-11):** в схеме `users` **паролей нет** — поля `telegram_id`,
> `tg_username`, `whatsapp_phone`, `channel_origin` (CHECK = telegram|whatsapp),
> `email_verification_code` (OTP). Регистрация идёт **через Telegram-бота** (admin/n8n-стек),
> далее планируется WhatsApp. Парольной аутентификации продукт не предусматривает.

**Целевая модель (M-9, не реализовано):** Telegram Mini App `initData` → клиент шлёт
подписанную строку → **сервер** (Edge Function/RPC) проверяет подпись секретом бота, находит
юзера по `telegram_id`, выдаёт сессию. Подпись проверяется только на сервере.

**Текущая (временный dev-вход):** реализован `MrsqmAuthService` на Supabase Auth
**email+пароль** — это **dev-костыль** для разработки/тестов (M-3-dev). 9 активным юзерам
заведены записи в `auth.users` (id = `public.users.id`, email-identity). При входе:
Supabase Auth → SELECT из `users` по `id = auth.uid()` с `is_active = true` (RLS
`users_select_own`); нет/деактивирован → `signOut` + ошибка. `mrsqmAuthGuard` закрывает все
`mrsqm/*`. **Удалить парольные записи перед продом.**

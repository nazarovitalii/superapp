...
- **Провайдер расшифровки:** бэкенд переключён на Groq `whisper-large-v3` (фолбэк OpenAI
  Whisper, если нет `GROQ_API_KEY`). `transcribe()` возвращает `{ text, provider }`; после
  голоса на ~4 сек мелькает метка `🟢 Groq/Whisper` над композером — видно, кто распознал.

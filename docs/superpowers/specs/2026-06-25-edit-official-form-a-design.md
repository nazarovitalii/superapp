...
| Обязательность полей  | **Все поля Form A обязательны** при подаче: Contract Number + дата начала + дата окончания + PDF (`listing_start`/`listing_end` NOT NULL в схеме → форма обязана их слать). В **add** — всегда; в **edit** — когда прикладывается новый Form A ИЛИ Pocket→Official (all-or-nothing). Пароль PDF — опционален («если есть»: PDF без пароля → нечего вводить). |

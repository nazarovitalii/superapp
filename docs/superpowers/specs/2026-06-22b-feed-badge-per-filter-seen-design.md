...
Вся работа — **read-side, наша сторона** (superapp-миграции + Angular-фронт). Репозиторий realtime, матчер и Delivery **не трогаем — бэкенд-задачи нет** (выяснено 2026-06-22).

- Фронт **не подписан** на realtime (поиск `.channel(`/`.subscribe(`/`postgres_changes` по `src/app/mrsqm` — ноль). Бейдж фронт **сам запрашивает** у `get_saved_filters`. Матчер фронту ничего не «шлёт» — он лишь пишет ряды в `filter_matches`, наша функция их читает.
- Зависим только от факта, что **ряды `filter_matches` существуют** (выход матчера, уже в проде). Колонку `matched_at` новая формула бейджа **не использует** (считаем по `NOT EXISTS user_filter_seen`).
...
- **Live-push (живой рост бейджа по вебсокету) — вне скоупа**, решено не делать сейчас (2026-06-22): не нужен фиче, реальная цена — инфраструктура вебсокетов на 30к, не код.

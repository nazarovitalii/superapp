-- ============================================================================
-- ⚠️ ЧЕРНОВИК — НЕ ПРИМЕНЯТЬ без «применяй». Применять ПЕРЕД пушем нового фронта
--   (текущий деплой шлёт p_occupancy_status строкой — после миграции это text[]).
--
-- Миграция: get_feed — заселённость МУЛЬТИСЕЛЕКТ (p_occupancy_status text -> text[]).
-- Снова смена сигнатуры → полный DROP+CREATE (не body-патч). База — живое тело get_feed v2.
-- ДЕЛЬТА: сигнатура p_occupancy_status text -> text[]; тело (COUNT+SELECT):
--   p.occupancy_status = p_occupancy_status -> p.occupancy_status = ANY(p_occupancy_status).
-- Применять под supabase_admin, в одной транзакции (--single-transaction -v ON_ERROR_STOP=1).
-- ОТКАТ: вернуть p_occupancy_status text + равенство (прежнее тело из database.md).
-- ============================================================================

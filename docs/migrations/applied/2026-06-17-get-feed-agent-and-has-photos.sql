-- Миграция: добавить в get_feed данные агента и флаг наличия фото.
-- Дата: 2026-06-17
-- Применять под ролью supabase_admin (CREATE OR REPLACE, идемпотентно).
--
-- ПРОБЛЕМА:
--   1) Колонка «Агент» в ленте всегда пустая («—») — get_feed не отдаёт ни имя
--      агента, ни агентство (нет JOIN на users/user_identities). [item 2]
--   2) Иконка 📷 в строке никогда не показывается — get_feed не отдаёт никакой
--      информации о фото объекта. [item 3]
--
-- РЕШЕНИЕ: в jsonb-вывод добавлены 4 поля:
--   owner_full_name, owner_agency_name, owner_photo_url, has_photos.
--   Логика фильтрации / COUNT / пагинации / сортировки НЕ изменена.
--
-- ВАЖНО (корректность ленты): owner_* берутся СКАЛЯРНЫМИ подзапросами, а НЕ
--   через LEFT JOIN, как в get_property. Причина: get_property — одна строка
--   (SELECT INTO), а лента многострочная; user_identities может иметь >1 строки
--   на юзера (несколько каналов) → прямой JOIN размножил бы строки ленты и
--   сломал бы count_total. Подзапрос с LIMIT 1 такого риска не несёт.
--
-- Базис — последняя живая версия (applied/2026-06-11-get-feed-add-community-name.sql),
-- воспроизведена целиком; добавлены только 4 поля в второй jsonb_build_object.
...
BEGIN
  ...
  -- Приоритет: 1) p_city_id параметр 2) user_identities.city_id
  -- ================================================================
  IF p_city_id IS NOT NULL THEN
    v_city_id := p_city_id;
  ELSE
    -- Берём из user_context (кэш) — быстрее чем JOIN через user_identities
  ...
  -- ================================================================
  -- ШАГ 3: Получить сеть юзера (для флага is_network)
  -- ================================================================
  SELECT array_cat(
  ...
  FROM (
    SELECT (
      ...
      jsonb_build_object(
        ...
        -- Агент (владелец): скалярные подзапросы — НЕ JOIN (см. шапку миграции)
        ...
        -- Есть ли фото у объекта (иконка 📷 в строке)

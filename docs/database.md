...
> ⚠️ **Тело ниже может быть устаревшим.** Актуальная формула `unseen_count` (**баг B фикс 2026-06-30** — global shown_at + my→0): `CASE WHEN sf.filters->>'scope'='my' THEN 0 ELSE <count filter_matches active, GREATEST(p.created_at,p.updated_at) > COALESCE(user_seen_listings.shown_at,'epoch')> END`. Теперь та же модель, что `get_feed.is_unseen` — просмотр объекта где угодно гасит бейдж (per-filter `user_filter_seen` БОЛЬШЕ НЕ участвует). Источник истины — живой `pg_get_functiondef` + `applied/2026-06-30-get-saved-filters-unseen-global-shown.sql`.
